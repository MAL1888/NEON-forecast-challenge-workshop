...
for(i in 1:length(lake_sites$field_site_id)) {  
  ...
  fit <- lm(site_target$temperature ~ poly(site_target$air_temperature, degree = 2) +
              poly(site_target$eastward_wind, degree = 2))
  ...
  for (t in 1:length(forecast_dates)) {
    ...
    forecasted_temperature <- param_df$beta1 + 
      ...
      poly(site_drivers$eastward_wind, degree = 2)[, 1] * param_df$beta4 +
      poly(site_drivers$eastward_wind, degree = 2)[, 2] * param_df$beta5 + 
