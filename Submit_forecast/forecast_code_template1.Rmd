...
subtitle: "MALoaiza first forecast submit"
...
```{r}
# I selected these two variables to forecast the temperature
met_variables <- c("air_temperature", "relative_humidity")
```
...
noaa_future_daily <- noaa_future |> 
  ...
  mutate(prediction = ifelse(variable == "air_temperature", prediction - 273.15, prediction)) |> 
  pivot_wider(names_from = variable, values_from = prediction) |> 
  select(any_of(c('datetime', 'site_id', met_variables, 'parameter')))# how can I add the variables "air_temperature, relative_humidit"
```
...
forecast_df <- NULL # Para limpiar cualquier dato en forcasat_df
