...
for(i in 1:length(lake_sites$field_site_id)) {  
  ...
  #model 1
 #fit_rh <- lm(site_target$temperature ~ site_target$air_temperature + site_target$relative_humidity)
  # Polynomial model
...
fit_summary <- summary(fit)
 ...
 sigma <- sd(residuals, na.rm = TRUE)
  ...
  forecasted_temperature <- param_df$beta1 + 
    poly(site_drivers$air_temperature, degree = 2)[, 1] * param_df$beta2 +
    poly(site_drivers$air_temperature, degree = 2)[, 2] * param_df$beta3 +
    poly(site_drivers$surface_downwelling_shortwave_flux_in_air, degree = 2)[,4] * param_df$beta4 +
    poly(site_drivers$surface_downwelling_shortwave_flux_in_air, degree = 2)[,5] * param_df$beta5 + 
